...
public class FacilityMaintenanceDAO {
	//setter
	public void setFacilityMaintenance(Request r, Facility f) throws SQLException {
		...
		if (f instanceof Group) {
			rPS.setInt(6, ((Group) f).getGroupID());
		} else if (f instanceof Building) {
			rPS.setInt(5, ((Building) f).getBuildingID());
		} else
			rPS.setInt(4, ((Room) f).getRoomID());
	 ...
	 // getter
	public void getFacilityMaintenance (int rID) throws SQLException {
		Statement st = DatabaseHelper.getConnection().createStatement();
		
		// get Request
		String st1 = "SELECT Request_ID, DateRequested, TextDescription, Room_ID, Building_ID,"
				+ " Group_ID FROM FacilityUsage WHERE Request_ID = " + rID;
		...
		while (schRS.next()) {
			...
			sch.setDateScheduled(new java.util.Date(schRS.getDate("DateScheduled").getTime()));
		}
		m.setSchedule(sch);
		schRS.close();
